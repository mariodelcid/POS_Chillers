...
const BLANK = { name: '', category: 'ingredient', unit: '', unitsPurchased: '' };
    ...
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <button
          ...
          style={{ padding: '8px 20px', background: '#1e293b', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: '0.9em' }}
        >
          {showForm ? 'Cancel' : '+ Add Item'}
        </button>
      </div>
      <p style={{ color: '#6b7280', marginTop: 4, marginBottom: 20 }}>All ingredients, packaging, and disposables.</p>
        ...
        <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: 20, marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
            <div>
              ...
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Azucar Regular"
            ...
            <div>
              ...
              <input value={form.unit} onChange={e => setForm(f => ({ ...f, unit: e.target.value }))} placeholder="e.g. kg, L, pcs"
                style={{ width: '100%', padding: '8px 10px', border: '1px solid #d1d5db', borderRadius: 6, fontSize: '0.88em', boxSizing: 'border-box' }} />
            </div>
            <div>
              <div style={{ fontSize: '0.75em', fontWeight: 600, color: '#374151', marginBottom: 4 }}>Units Purchased *</div>
              <input type="number" value={form.unitsPurchased} onChange={e => setForm(f => ({ ...f, unitsPurchased: e.target.value }))} placeholder="e.g. 25" min="0"
                style={{ width: '100%', padding: '8px 10px', border: '1px solid #d1d5db', borderRadius: 6, fontSize: '0.88em', boxSizing: 'border-box' }} />
            </div>
          </div>
          {error && <div style={{ color: '#dc2626', fontSize: '0.82em', marginBottom: 10 }}>{error}</div>}
          <button onClick={handleAdd} disabled={saving}
            style={{ padding: '8px 24px', background: '#059669', color: 'white', border: 'none', borderRadius: 6, cursor: saving ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: '0.88em', opacity: saving ? 0.7 : 1 }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        {['all', 'ingredient', 'packaging', 'disposable'].map(c => (
          <button key={c} onClick={() => setFilterCat(c)} style={{
            ...
            background: filterCat === c ? '#1e293b' : 'white', color: filterCat === c ? 'white' : '#374151'
      ...
      {filtered.length === 0 ? (
        <div style={{ padding: 40, textAlign: 'center', color: '#9ca3af' }}>No items found. Add one above or use the Edit page.</div>
      ) : (
        categoryOrder.filter(cat => grouped[cat]?.length > 0).map(cat => (
          <div key={cat} style={{ marginBottom: 32 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8, padding: '8px 14px',
              background: (CAT_COLOR[cat] || '#374151') + '15', borderLeft: `4px solid ${CAT_COLOR[cat] || '#374151'}`, borderRadius: '0 6px 6px 0' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2.5fr 120px 160px', gap: 12, padding: '8px 14px',
              fontWeight: 700, fontSize: '0.78em', color: '#374151', borderBottom: '2px solid #e5e7eb', background: '#f8fafc', borderRadius: '6px 6px 0 0' }}>
              <div>Name</div><div>Unit</div><div>Units Purchased</div>
            </div>
            {grouped[cat].map((item, idx) => {
              const qty = item.unitsPurchased || 1;
              return (
                <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '2.5fr 120px 160px', gap: 12, padding: '10px 14px',
                  alignItems: 'center', borderBottom: '1px solid #f3f4f6', background: idx % 2 === 0 ? 'white' : '#fafafa' }}>
                  <div style={{ fontWeight: 600, fontSize: '0.92em' }}>{item.name}</div>
                  <div style={{ fontSize: '0.85em', color: '#6b7280' }}>{item.unit}</div>
                  <div style={{ fontSize: '0.85em' }}>{qty}{item.presentation && item.presentation !== item.unit ? <span style={{ color: '#6b7280' }}> ({item.presentation})</span> : ''}</div>
                </div>
              );
            })}
          </div>
      ...
      {filtered.length > 0 && (
        <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '14px 20px', background: '#1e293b', color: 'white', borderRadius: 8, fontWeight: 700, fontSize: '0.92em' }}>
