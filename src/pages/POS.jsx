  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', height: 'calc(100vh - 60px)', gap: 0 }}>
      {showCelebration && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ textAlign: 'center', color: '#ffffff' }}>
            <div style={{ fontSize: '360px', fontWeight: '900', marginBottom: '20px', color: '#22c55e' }}>
              {celebrationAmount}
            </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', borderRight: '2px solid #e5e7eb', height: '100%', overflow: 'hidden' }}>
        <div style={{ padding: '20px', overflow: 'auto', flex: 1, backgroundColor: '#ffffff', height: '100%' }}>
                ...
                {list.map((it) => {
                  ...
                  return (
                    <button key={it.id} onClick={() => addToCart(it)}
                      style={{ padding: '12px', textAlign: 'left', border: `2px solid ${borderColor}`, borderRadius: '12px', background: backgroundColor, cursor: 'pointer', fontSize: '16px', transition: 'all 0.2s', display: 'flex', flexDirection: 'column', gap: '4px', minHeight: '80px', justifyContent: 'center' }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', backgroundColor: '#f8fafc', padding: '20px', height: '100%', overflow: 'auto' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', backgroundColor: '#ffffff', borderRadius: '12px', padding: '16px', marginBottom: '20px', border: '1px solid #e5e7eb', minHeight: '200px', maxHeight: '400px' }}>
          {cart.length === 0 ? (
        ...
        <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '20px', marginBottom: '20px', border: '1px solid #e5e7eb' }}>
          ...
          <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 16px', border: `2px solid ${paymentMethod === 'cash' ? '#2563eb' : '#e5e7eb'}`, borderRadius: '8px', backgroundColor: paymentMethod === 'cash' ? '#eff6ff' : '#ffffff', cursor: 'pointer', flex: 1, justifyContent: 'center', fontWeight: '500' }}>
              <input type="radio" name="pm" value="cash" checked={paymentMethod === 'cash'} onChange={() => setPaymentMethod('cash')} style={{ margin: 0 }} />
              Cash
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 16px', border: `2px solid ${paymentMethod === 'credit' ? '#2563eb' : '#e5e7eb'}`, borderRadius: '8px', backgroundColor: paymentMethod === 'credit' ? '#eff6ff' : '#ffffff', cursor: 'pointer', flex: 1, justifyContent: 'center', fontWeight: '500' }}>
          ...
          {paymentMethod === 'credit' && (
            <div style={{ padding: '16px', backgroundColor: '#f0fdf4', borderRadius: '8px', border: '1px solid #22c55e', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '18px', fontWeight: '600', color: '#059669', marginBottom: '8px' }}>Credit Payment Ready</div>
              <div style={{ fontSize: '14px', color: '#6b7280' }}>Click "Complete Order" to finalize the transaction</div>
            </div>
          )}
        </div>
        {paymentMethod === 'credit' && (
          <button
            ...
            style={{ width: '100%', padding: '16px', background: cart.length === 0 || submitting ? '#9ca3af' : '#3b82f6', color: '#ffffff', border: 'none', borderRadius: '12px', fontSize: '18px', fontWeight: '700', cursor: cart.length === 0 || submitting ? 'not-allowed' : 'pointer', transition: 'all 0.2s', marginBottom: '12px' }}
          >
            Process Credit Card
          </button>
        )}
        <button
          ...
          style={{ width: '100%', padding: '20px', background: cart.length === 0 || submitting || (paymentMethod === 'cash' && tenderCents < totalCents) ? '#9ca3af' : '#059669', color: '#ffffff', border: 'none', borderRadius: '12px', fontSize: '20px', fontWeight: '700', cursor: cart.length === 0 || submitting || (paymentMethod === 'cash' && tenderCents < totalCents) ? 'not-allowed' : 'pointer', transition: 'all 0.2s', marginBottom: '12px' }}
        >
            ...
            <div style={{ padding: '16px', border: '2px solid #dc2626', borderRadius: '12px', backgroundColor: '#fef2f2' }}>
              ...
              <div style={{ marginBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 12px', border: `2px solid ${purchasePaymentMethod === 'cash' ? '#dc2626' : '#d1d5db'}`, borderRadius: '6px', backgroundColor: purchasePaymentMethod === 'cash' ? '#fef2f2' : '#ffffff', cursor: 'pointer', fontWeight: '500' }}>
                    <input type="radio" name="purchasePaymentMethod" value="cash" checked={purchasePaymentMethod === 'cash'} onChange={(e) => setPurchasePaymentMethod(e.target.value)} style={{ margin: 0 }} />
                    Cash
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 12px', border: `2px solid ${purchasePaymentMethod === 'card' ? '#dc2626' : '#d1d5db'}`, borderRadius: '6px', backgroundColor: purchasePaymentMethod === 'card' ? '#fef2f2' : '#ffffff', cursor: 'pointer', fontWeight: '500' }}>
                    <input type="radio" name="purchasePaymentMethod" value="card" checked={purchasePaymentMethod === 'card'} onChange={(e) => setPurchasePaymentMethod(e.target.value)} style={{ margin: 0 }} />
                    Card
                  </label>
                </div>
              </div>
              <div style={{ marginBottom: '12px' }}>
                ...
                {purchaseReceipt && (
                  <div style={{ marginTop: '4px', fontSize: '12px', color: '#059669', fontWeight: '500' }}>Selected: {purchaseReceipt.name}</div>
                )}
              </div>
        ...
        {message && (
          <div style={{ marginTop: '16px', padding: '12px', backgroundColor: message.includes('complete') ? '#f0fdf4' : '#fef2f2', border: `1px solid ${message.includes('complete') ? '#22c55e' : '#dc2626'}`, borderRadius: '8px', color: message.includes('complete') ? '#059669' : '#dc2626', fontSize: '16px', fontWeight: '500', textAlign: 'center' }}>
            {message}
          </div>
        )}
