...
export default function POS() {
  ...
  const [celebrationMethod, setCelebrationMethod] = useState('cash');
        ...
        const autoComplete = async () => {
          try {
            ...
            if (res.ok) {
              ...
              setCelebrationMethod(savedPaymentMethod);
  ...
  async function completeOrder() {
    ...
    try {
      ...
      setCelebrationAmount(centsToUSD(data.totalCents));
      setCelebrationMethod(paymentMethod);
      setShowCelebration(true);
      setTimeout(() => { setShowCelebration(false); setCelebrationAmount(''); }, 2500);
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', height: 'calc(100vh - 60px)', gap: 0 }}>
      {showCelebration && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: celebrationMethod === 'cash' ? '#15803d' : '#b91c1c', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ textAlign: 'center', color: '#fff' }}>
            <div style={{ fontSize: '360px', fontWeight: '900', marginBottom: '20px' }}>{celebrationAmount}</div>
            <div style={{ fontSize: '48px', fontWeight: '700' }}>SALE COMPLETE!</div>
            <div style={{ fontSize: '32px', fontWeight: '600', marginTop: '12px', opacity: 0.85 }}>{celebrationMethod === 'cash' ? '💵 CASH' : '💳 CREDIT'}</div>
          </div>
        </div>
      )}
      {showSquareConfirm && (
